 ...
 */
public class JsoupUtil implements Connection {
	...
	@Contract(pure = true) public static Connection connect(@NotNull String url) {
		return JsoupUtil.config().create(url);
	}
	 ...
	 * @return 此连接，用于链接
	 */
	@Contract(pure = true) protected Connection create(@NotNull String url) {
		conn = Jsoup.connect(this.url = url);
		conn = conn.maxBodySize(0).timeout(0).ignoreContentType(true).ignoreHttpErrors(true);
	...
	@Contract(pure = true) public Connection postDataCharset(String charset) {

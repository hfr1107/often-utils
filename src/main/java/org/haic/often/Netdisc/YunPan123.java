...
public class YunPan123 {
	 ...
	 * @return 包含文件信息的JSON数据格式列表
	 */
	@Contract(pure = true) public static List<JSONObject> getInfosAsPage(@NotNull String shareUrl, @NotNull String sharePwd) {
		return getInfosAsPage(shareUrl.substring(shareUrl.lastIndexOf("/") + 1), sharePwd, "0", 1, "/");
	}

	@Contract(pure = true) protected static List<JSONObject> getInfosAsPage(String key, String sharePwd, String parentId, int page, String path) {
		...
		for (int i = 0; i < files.size(); i++) {
			JSONObject info = files.getJSONObject(i);
	 ...
	 * @return Map - 文件名, 文件直链
	 */
	@Contract(pure = true) public static Map<String, String> getStraightsAsPage(@NotNull String shareUrl, @NotNull String sharePwd) {
		return getInfosAsPage(shareUrl, sharePwd).stream()
				.collect(Collectors.toMap(l -> l.getString("Path") + l.getString("FileName"), l -> l.getString("DownloadUrl")));
	}
	 ...
	 * @return Map - 文件名, 文件直链
	 */
	@Contract(pure = true) public static Map<String, String> getStraightsAsPage(@NotNull String shareUrl) {
	...
	public static class YunPan123API {
		...
		@Contract(pure = true) public List<JSONObject> listShares(@NotNull String search) {
			return JSONObject.parseObject(
					...
					.getJSONObject("data").getJSONArray("InfoList").toJavaList(JSONObject.class);
		}
		 ...
		 * @return 文件列表
		 */
		@Contract(pure = true) protected List<JSONObject> getInfoListAsHome(@NotNull String fileId, @NotNull String search, boolean trashed) {
			Map<String, String> data = new HashMap<>();
			data.put("driveId", "0");
			data.put("limit", "1000");
			data.put("next", "0");
			...
			return filesInfo.toJavaList(JSONObject.class);
		}
		 ...
		 * @param folderPath 存放的文件夹路径
		 */
		@Contract(pure = true) public void download(@NotNull List<JSONObject> fileInfos, @NotNull String folderPath) {
